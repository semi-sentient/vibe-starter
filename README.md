...
Releases are cut **locally**, never by CI. `npm run release` reads the conventional commits since the last `vX.Y.Z` tag, prepends the new CHANGELOG section, bumps the version, commits, tags, pushes, and creates the GitHub release — all under your own `git` and `gh` credentials. No bot ever pushes a commit or opens a pull request on your behalf. It refuses (with the reason, and there are six of them) on a dirty working tree, on a branch other than the default one, when nothing release-worthy has landed since the last tag, or when a previous release was bumped locally but never pushed. If you have run `npm run setup:github`, the required checks it puts on `main` do not stand in the way: the admin role is a bypass actor on that ruleset precisely so the release push goes through — the reasoning, and what it costs, is in [Branch protection](DEPLOY.md#branch-protection).
...
The rules the agent follows are written down in [`docs/agents/shipping.md`](docs/agents/shipping.md); the one-time human setup — Railway, GitHub settings, going live — is in the [deploy runbook](DEPLOY.md).
