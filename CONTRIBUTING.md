...
Releases themselves are **local, not CI**: `npm run release` reads the conventional commits since the last `vX.Y.Z` tag with git-cliff, writes the `CHANGELOG.md` section, bumps the version, tags, pushes, and publishes the GitHub release — all under your own `git`/`gh` credentials. Run it from `main`, on a clean tree, after this pass is green. It refuses (and says why) for six distinct reasons — a dirty tree, off the default branch, nothing since the last tag mapping to a changelog section, unreadable tags, a malformed version, or a previous release that was bumped locally and never pushed. Rationale for the local-not-CI shape is in [`docs/design/TOOLING_DESIGN.md`](docs/design/TOOLING_DESIGN.md#versioning-automation). On a repo that has had `npm run setup:github` applied, the release's push to `main` **goes through for an admin**: `main-required-checks` carries a bypass for the admin repository role. If you are not an admin on the repo, that push is refused — see the recovery recipe in [`DEPLOY.md`](DEPLOY.md#branch-protection). Verified against a live repo. The push is `git push --atomic --follow-tags`, and `--atomic` is load-bearing — a ruleset that refuses the branch update still lets `--follow-tags` land the tag on its own, publishing a `vX.Y.Z` no remote branch contains; `--atomic` makes the pair one transaction, so either both refs move or neither does. Detail in [`DEPLOY.md`](DEPLOY.md#branch-protection).
...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) has four jobs. Their **display names** matter as much as their ids: three of them are named verbatim in `main-required-checks`, one of the two rulesets `npm run setup:github` installs (see [`DEPLOY.md`](DEPLOY.md#branch-protection)), and GitHub will wait forever for a required check whose name no job produces — so renaming one blocks every PR. `ci.yml` carries a contract comment above each; `scripts/setup-github.mjs` reads the names back out of the file and, if one has gone missing, refuses to apply anything at all — both rulesets and the merge settings.
...
- **`docker-smoke`** → display name **`Docker smoke`** _(required)_ — builds both production Dockerfiles and runs the real stack (Postgres + api + nginx web) on a Docker network, then asserts through the web container's proxy: `/` and `/login` return 200, the served HTML contains the `id="root"` mount point and references at least one JS URL, **every one of those URLs this stack is responsible for serving comes back with a JavaScript content type**, and `/api/health` reports `"db":"up"` plus a `"version"` field. "Responsible for" means root-relative paths plus absolute URLs on the smoke stack's own origin (the origin is stripped and they are checked identically — a Vite `base` can emit either form); a URL on a genuinely different host is skipped and logged, because prefixing our origin onto it would build nonsense. If **every** URL was skipped the step fails rather than passing having verified nothing. The content type is what matters, not the status: the SPA fallback answers 200 with `index.html` for a missing asset, so status alone would pass a blank page. That last one proves the bundled `package.json` import survives into a runtime image that contains no `package.json`. It is the "green but undeployable" gate — `Build & test` proves the code compiles and its tests pass, which is not the same as "the images Railway builds actually boot".
