...
| Changelog        | Keep-a-Changelog format, generated by `npm run release` (git-cliff)         | This doc    |
...
- **Deployment:** Railway as the deploy target, deploying from `main` behind required CI checks. Railway's per-pull-request preview deployments are supported but **opt-in** — a dashboard toggle the builder turns on if they want one, not a default. (See `TOOLING_DESIGN.md`.)
...
### Keep-a-Changelog, generated by a local release command
...
The changelog is generated from the conventional commits on `main`, but the generation is a **deliberate local act, not CI automation**: `npm run release` runs git-cliff over the commits since the last `vX.Y.Z` tag, prepends the new section, bumps the version, tags, pushes, and publishes the GitHub release — under the invoker's own `git`/`gh` credentials. No bot opens a PR and no workflow cuts a tag. The rationale, including the two failure modes that retired the previous bot-PR approach, is in `TOOLING_DESIGN.md` → Versioning automation. Zero manual changelog maintenance either way; the difference is who presses the button.

This pairs well with the `commit` skill from the pre-installed skills pipeline, which produces conventional-commit messages (invoked automatically by `run-plan` after each phase) — those messages are exactly what the release command reads.
