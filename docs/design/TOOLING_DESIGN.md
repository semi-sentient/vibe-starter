...
Why `0.1.0`: a `0.0.0` manifest is release-please's "never released yet" sentinel, and for `release-type: node` the [documented default first-release version is already `0.1.0`](https://github.com/googleapis/release-please/blob/main/docs/manifest-releaser.md#bootstrapping) — `initial-version: "0.1.0"` ([config-schema key](https://github.com/googleapis/release-please/blob/main/schemas/config.json)) pins that baseline explicitly so the first release lands on `0.1.0` regardless of strategy defaults rather than being inferred from commit history.

If the first release PR in a generated repo does _not_ come out as `0.1.0` (e.g. release-please infers a different bump), the deterministic fallback is to add a `Release-As: 0.1.0` footer to a commit on that first release PR, which pins the release to `0.1.0`.
