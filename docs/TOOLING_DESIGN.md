...
**GitHub template repo** distribution. **5-command Quick Start** (clone, copy `.env.example`, `docker compose up`, `npm install`, `npm run dev`). **Idempotent `scripts/bootstrap.sh`** for project-name replacement and secret generation. **`predev` script** auto-runs migrations before dev server starts.
...
# 1. Replace placeholder project name in package.json
if grep -q '"name": "vibe-starter"' package.json; then
  read -p "Project name: " PROJECT_NAME
  sed -i.bak "s/vibe-starter/$PROJECT_NAME/g" package.json
  rm package.json.bak
fi

# 2. Generate SESSION_SECRET if .env doesn't have one
...
echo "  - Edit .env with your DATABASE_URL and other vars"
...
flowchart LR
  ...
  RPPR -->|merged| Tag[Tagged release<br/>v1.X.Y]
